...
function ProjectSiteEdit({ match, history }) {
    ...
    useEffect(() => {
        projectSites
            ...
            .then((res) => {
                ...
                arrayList.forEach((e) => delete obj[e])
    ...
    }, [match.params.id, match.params.site_id])
    useEffect(() => {
        sites
            .getAll({
                params: {
    ...
    const submit = (e) => {
        e.preventDefault()
        projectSites.update(data).then((res) => {
            toast.success('Succes update data sites')
